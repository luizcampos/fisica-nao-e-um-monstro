...
$select = "SELECT * FROM usuario where nome_usu LIKE '%$pesquise%'  ORDER BY denuncias_usu DESC";
try {
...
<div class="tabela">
 ...
 Denuncias:<?php echo " ".$mostra->denuncias_usu;?>
          <?php if ($mostra->status_usu==1){?> <form method="post" action="includes/bloquear2.php?id=<?php echo $mostra->id_usu?>"> <input class="bloquear grow" id="bloquear"  type="submit" name="bloquear" value="Bloquear"></form>
			<?php }else{
				?> <form method="post" action="includes/bloquear2.php?id=<?php echo $mostra->id_usu?>"> <input class="desbloquear grow" id="desbloquear"  type="submit" name="desbloquear" value="Desbloquear"<?php }?>
