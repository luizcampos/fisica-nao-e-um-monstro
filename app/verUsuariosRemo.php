...
<form name="top" method="post" action="barraPesquisa5.php?pg=1">
...
if (!is_numeric($pg)){echo '<script language= "JavaScript">
				location.href="verUsuariosRemo.php?pg=1";
				</script>';}
...
$select = "SELECT * from usuario where status_usu=2 ORDER BY denuncias_usu DESC LIMIT $inicio, $qtd ";
	try {
...
<form method="post" action="includes/removerUsu.php?id=<?php echo $mostra->id_usu?>"> <input class="desbloquear grow" id="remover"  type="submit" name="remover" value="Remover" />
	...
	<?php
			  ...
			  echo '<br/><br/><div id="erro"><img src="imagem/Icone/warning.png" /> Desculpe, não existem usuários bloqueados </div>';
...
<?php
 ...
 ?>
<!--abre paginas-->
<div class="paginas" id="paginas" > <a href="verUsuariosRemo.php?pg=1" >&laquo;Inicio</a>
<?php
  if(isset($_GET['pg'])){
	  $numPag = $_GET['pg'];
	  }
...
?>
     <a href="verUsuariosRemo.php?pg=<?php echo $i?>" class="ativo<?php echo $i; ?>"><?php echo $i;?></a>     
...
?>
<a href="verUsuariosRemo.php?pg=<?php echo $paginas;?>" >Final&raquo;</a> 
</div>
